...
const App: React.FC = () => {
        ...
        <div className="max-w-6xl mx-auto">
          <header className="flex items-center justify-between mb-12">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold">Veo Studio</h1>
                <p className="text-zinc-400 text-sm">Video Production Platform</p>
              </div>
            </div>
            <button 
          ...
          {showNewProjectModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
              <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 max-w-md w-full shadow-2xl animate-in fade-in zoom-in duration-200">
                <h2 className="text-xl font-bold mb-4">Create New Project</h2>
                <input
                  autoFocus
                  type="text"
                  placeholder="Project Name (e.g. My Sci-Fi Short)"
                  className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-4 py-3 mb-6 focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 text-white"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {projects.length === 0 ? (
              <div className="col-span-full text-center py-20 border border-dashed border-zinc-800 rounded-2xl bg-zinc-900/30">
                ...
                <p className="text-zinc-600 mb-6">Create your first video project to get started.</p>
                <button 
                  onClick={() => setShowNewProjectModal(true)}
              ...
              projects.map(project => (
                <div 
                  ...
                  className="group relative bg-zinc-900 border border-zinc-800 hover:border-zinc-600 rounded-xl overflow-hidden cursor-pointer transition-all hover:-translate-y-1 hover:shadow-xl"
                >
                  <div className="aspect-video bg-zinc-800 relative">
                    {project.characterImageBase64 ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-zinc-800 to-zinc-900">
                        <Clapperboard className="w-12 h-12 text-zinc-700 group-hover:text-zinc-600 transition-colors" />
                      </div>
                    )}
                    <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button 
                        onClick={(e) => deleteProject(e, project.id)}
                        className="p-2 bg-black/50 hover:bg-red-500/80 rounded-full text-white backdrop-blur-sm transition-colors"
                      >
                  ...
                  <div className="p-5">
                    <h3 className="font-bold text-lg text-white mb-1 truncate">{project.name}</h3>
                    ...
                    <div className="flex items-center gap-2">
                       <span className={`px-2 py-0.5 rounded text-[10px] uppercase font-bold tracking-wider ${
                         project.step === 'production' ? 'bg-emerald-500/10 text-emerald-500' :
                         project.step === 'character' ? 'bg-indigo-500/10 text-indigo-500' :
  ...
  return (
    <div className="flex h-screen bg-zinc-950 text-white overflow-hidden">
      ...
      {/* Sidebar Navigation */}
      <aside className="w-16 md:w-20 border-r border-zinc-800 flex flex-col items-center py-6 bg-zinc-900/50">
        <button 
          onClick={() => setCurrentProjectId(null)}
          className="w-10 h-10 mb-6 rounded-lg flex items-center justify-center text-zinc-400 hover:text-white hover:bg-zinc-800 transition-all"
          title="Back to Projects"
        >
          <ArrowLeft size={20} />
        </button>

        <div className="w-10 h-10 bg-indigo-500/10 rounded-xl flex items-center justify-center mb-8">
        ...
        <div className="mt-auto mb-4">
           <button 
             ...
             className="w-10 h-10 rounded-lg flex flex-col items-center justify-center text-zinc-500 hover:text-white hover:bg-zinc-800 transition-all"
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden">
        ...
        {/* Header */}
        <header className="h-16 border-b border-zinc-800 flex items-center justify-between px-6 bg-zinc-900/30 backdrop-blur-sm z-10">
          <div className="flex items-center gap-3">
             <span className="text-zinc-500 font-mono text-xs uppercase tracking-wider">Project</span>
             <h1 className="text-lg font-bold text-zinc-200">{currentProject.name}</h1>
          </div>
          {loading && (
          ...
          {currentProject.step === 'script' && (
            <div className="max-w-2xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="text-center space-y-2">
                <h2 className="text-3xl font-bold tracking-tight">Main Project Script</h2>
                <p className="text-zinc-400">Describe the full script or concept for your video project. Gemini will break this down into individual scenes for production.</p>
              </div>
              
              <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-2 shadow-xl">
                <textarea
                  className="w-full bg-transparent border-none text-lg p-4 focus:ring-0 min-h-[200px] resize-none placeholder-zinc-600"
                ...
                />
                <div className="flex justify-between items-center px-4 pb-2 pt-2 border-t border-zinc-800/50">
                   <div className="flex gap-2">
                     <span className={`text-xs px-2 py-1 rounded bg-zinc-800 ${currentProject.aspectRatio === '16:9' ? 'text-white' : 'text-zinc-500'}`}>16:9</span>
                     <span className={`text-xs px-2 py-1 rounded bg-zinc-800 ${currentProject.aspectRatio === '9:16' ? 'text-white' : 'text-zinc-500'}`}>9:16</span>
                   </div>
                   <button
                    disabled={loading || !currentProject.scriptPrompt}
                    onClick={handleGenerateScript}
                    className="bg-white text-black hover:bg-zinc-200 px-4 py-2 rounded-lg font-semibold flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                  >
              ...
              {currentProject.scenes.length > 0 && (
                <div className="space-y-4">
                  <h3 className="text-sm font-semibold text-zinc-500 uppercase tracking-wider">Generated Scenes</h3>
                  <div className="space-y-3">
                    {currentProject.scenes.map((scene, idx) => (
                      <div key={scene.id} className="group flex gap-4 p-4 bg-zinc-900/50 border border-zinc-800 hover:border-zinc-700 rounded-xl transition-all">
                        <div className="w-8 h-8 rounded-full bg-zinc-800 flex items-center justify-center text-xs text-zinc-400 font-mono">
                          {idx + 1}
                        </div>
                        <input
                          type="text"
                          ...
                          className="flex-1 bg-transparent border-none p-0 focus:ring-0 text-sm text-zinc-200 placeholder-zinc-600"
                        />
                      </div>
                    ))}
                  </div>
          ...
          {currentProject.step === 'character' && (
            <div className="max-w-4xl mx-auto h-full flex flex-col md:flex-row gap-8 animate-in fade-in slide-in-from-right-4 duration-500">
              <div className="flex-1 space-y-6">
                <div>
                   <h2 className="text-2xl font-bold mb-2">Design your Star</h2>
                   <p className="text-zinc-400 text-sm">Create a consistent character to feature in your videos. This image will be used as the starting reference for Veo.</p>
                </div>

                <div className="space-y-3">
                  <label className="text-sm font-medium text-zinc-300">Character Description</label>
                  <div className="flex gap-2">
                    <input 
                      type="text"
                      className="flex-1 bg-zinc-900 border border-zinc-800 rounded-lg px-4 py-2.5 focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500"
                ...
                <div className="bg-zinc-900/50 rounded-xl p-4 border border-zinc-800">
                  <h3 className="text-sm font-semibold text-zinc-500 mb-4">Script Preview</h3>
                  <div className="space-y-2 max-h-60 overflow-y-auto pr-2">
                    {currentProject.scenes.map((s, i) => (
                      <div key={s.id} className="text-sm text-zinc-400 flex gap-3">
              ...
              <div className="w-full md:w-80 flex flex-col gap-4">
                 <div className="aspect-square w-full bg-zinc-900 border-2 border-dashed border-zinc-800 rounded-2xl flex items-center justify-center overflow-hidden relative">
                    {currentProject.characterImageBase64 ? (
                      <>
                        <img 
            ...
            <div className="flex flex-col h-full gap-8 animate-in fade-in duration-500">
              ...
              <div className="flex-1 flex flex-col min-h-0">
                <div className="flex items-center justify-between mb-4">
                   <h2 className="text-xl font-bold">Video Timeline</h2>
                   <div className="flex gap-2">
                      <button onClick={addScene} className="text-xs flex items-center gap-1 px-3 py-1.5 bg-zinc-800 hover:bg-zinc-700 rounded text-zinc-300 transition-colors">
                        <Plus className="w-3 h-3" /> Add Scene
                      </button>
                   </div>
                </div>
                
                <div className="flex-1 overflow-hidden bg-zinc-950/50 rounded-2xl border border-zinc-800/50 p-6 flex flex-col justify-center">
                   {currentProject.scenes.length === 0 ? (
                     <div className="text-center text-zinc-500">
                       <p>No scenes in timeline.</p>
                       <button onClick={addScene} className="mt-2 text-indigo-400 hover:underline">Add one</button>
                     </div>
                   ) : (
                     <Timeline 
              ...
              {/* Character Ref */}
              <div className="h-32 bg-zinc-900 border-t border-zinc-800 p-4 flex gap-4 items-center">
                 <div className="w-24 h-24 rounded-lg bg-zinc-800 overflow-hidden flex-shrink-0 relative">
                    {currentProject.characterImageBase64 && (
                      <img 
                        src={`data:image/png;base64,${currentProject.characterImageBase64}`} 
                        className="w-full h-full object-cover" 
                 ...
                 <div>
                   <h3 className="text-sm font-medium text-white">Active Character Reference</h3>
                   <p className="text-xs text-zinc-500 mt-1 max-w-md">
                     This character image is being used as the start frame for all Veo generations to maintain consistency.
                   </p>
                 </div>
                 <div className="ml-auto text-right">
                    <div className="text-xs text-zinc-500 uppercase tracking-wider font-bold mb-1">Total Scenes</div>
                    <div className="text-2xl font-mono text-white">{currentProject.scenes.length}</div>
                 </div>
              </div>
            </div>
          )}
...
const NavItem: React.FC<NavItemProps> = ({ active, icon, label, onClick }) => (
  ...
  >
    <div className={`p-2 rounded-lg ${active ? 'bg-indigo-500/10' : ''}`}>
      {icon}
    </div>
